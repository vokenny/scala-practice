...
object SecretSanta extends App {
  ...
  // Keeps reshuffling until the final list generates all valid pairs, terrible for large list sizes, but fine for small
  ...
  //Test
  //showResult(pairNamesRandomly((1 to 10).map(e => e.toString).toList))
}
